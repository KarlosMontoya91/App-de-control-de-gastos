      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 animate-in fade-in duration-500">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center space-y-2">
            <h1 className="text-3xl font-bold text-white tracking-tight">FinFlow <span className="text-yellow-400">2026</span></h1>
            <p className="text-slate-400">Setup Inicial</p>
          </div>
          <Card className="p-6 space-y-6">
            ...
            {step === 3 && (
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2">
                ...
                {formData.recurring && formData.recurring.length > 0 && (
                  <div className="text-xs text-slate-500 mt-2">
                    <p className="font-semibold mb-1">Tus fijos incluyen:</p>
                    <p>{formData.recurring.map(r => r.name).join(', ')}</p>
                  </div>
                )}
              ...
              <Card key={b.id} className="p-4 relative overflow-visible transition-all hover:border-slate-700">
                {isOver && (
                  <div className="absolute -top-2 -right-2 group z-10">
                    ...
                    <div className="absolute right-0 -top-10 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-white text-xs px-2 py-1.5 rounded whitespace-nowrap border border-slate-700 shadow-xl pointer-events-none z-20">
                       Excedido por <span className="font-bold text-red-400">{formatCurrency(overAmount)}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-between items-end mb-2">
                   <div>
                     <h3 className="text-white font-bold text-lg flex items-center gap-2">
                       {b.name}
                       {isNearLimit && <div className="w-2 h-2 rounded-full bg-yellow-400 animate-pulse" title="Cerca del límite"/>}
                     </h3>
                     <p className={`text-xs ${isOver ? 'text-red-400 font-bold' : isNearLimit ? 'text-yellow-400' : 'text-slate-400'}`}>
                       {isOver ? 'Has excedido el presupuesto' : isNearLimit ? '¡Cuidado! Cerca del límite' : 'Bajo control'}
                     </p>
                   </div>
                ...
                <div className="mt-2 text-right text-[10px] text-slate-500 uppercase tracking-widest font-semibold">
        ...
        <Card className="p-4 bg-blue-500/10 border-blue-500/20 mt-6">
           <div className="flex gap-3">
             ...
             <p className="text-xs text-blue-200 leading-relaxed">
               ...
               Al marcar un gasto como pagado, se descontará automáticamente de tu <strong>Disponible Real</strong> en el inicio. Esto te ayuda a visualizar el impacto en tu flujo de efectivo.
             </p>
           </div>
        </Card>
      </div>
  ...
  const AddTransactionModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-0 sm:p-4">
        <div className="bg-slate-900 w-full max-w-md sm:rounded-2xl rounded-t-2xl border-t sm:border border-slate-800 shadow-2xl animate-in slide-in-from-bottom-10">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {DEFAULT_CATEGORIES.slice(0, 5).map(cat => (
                  <button type="button" key={cat.name} onClick={() => setCategory(cat.name)} className={`text-xs py-2 px-1 rounded border transition-colors ${category === cat.name ? 'border-yellow-400 text-yellow-400 bg-yellow-400/10' : 'border-slate-800 text-slate-400'}`}>{cat.name}</button>
                ))}
                <button type="button" className="text-xs py-2 border border-dashed border-slate-700 text-slate-500 rounded">+ Otra</button>
              </div>
            </div>
